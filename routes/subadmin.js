        ...
        checkSchema({
            ...
            if (!results.isEmpty()) {
                ...
                db.getOrgId(req.user.id)
                    .then((result) => {
                        orgID = result.rows[0].organization;
                        db.registerUser(req.body.userName, orgID, req.body.email, req.body.password, req.body.mobile, req.body.address, req.body.city, req.body.country, req.body.state, req.body.postcode, 'www.google.com' ,'Decription not Provided','',false,req.body.isActive,req.body.user_type)
                        .then((data) =>{
                            console.log(data)
...
router.get('/activeuserlist',(req,res)=>{
    db.postgreDatabase.query("select * from user_table where isactive = true AND user_type=$1 OR user_type=$2", [`mentor`, `student`])
    .then((results)=>{
        console.log(results.rows);
        res.send(results.rows);
    }).catch(err=>{
...
router.get('/inactiveuserlist',(req,res)=>{
    db.postgreDatabase.query("select * from user_table where isactive = false AND user_type=$1 OR user_type=$2", [`mentor`, `student`])
    .then((results)=>{
        console.log(results.rows);
        res.send(results.rows);
    }).catch(err=>{
